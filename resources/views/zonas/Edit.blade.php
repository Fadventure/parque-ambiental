    ...
    <div class="py-8">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white rounded-lg shadow-sm p-6">
                ...
                <form method="POST" action="{{ route('zonas.update', $zona->id) }}" class="space-y-5">
                    @csrf
                    @method('PUT')

                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Nombre de la zona</label>
                        <input type="text" name="nombre" value="{{ old('nombre', $zona->nombre) }}"
                        ...
                        <select name="administrador_id" class="w-full border border-gray-300 rounded-md text-sm px-3 py-2">
                            ...
                            @foreach($administradores as $admin)
                                <option value="{{ $admin->id }}"
                                    {{ old('administrador_id', $zona->administrador_id) == $admin->id ? 'selected' : '' }}>
                                    {{ $admin->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="number" step="0.1" name="humedad" value="{{ old('humedad', $zona->humedad) }}"
                                   class="w-full border border-gray-300 rounded-md text-sm px-3 py-2">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Temperatura (°C)</label>
                            <input type="number" step="0.1" name="temperatura" value="{{ old('temperatura', $zona->temperatura) }}"
                                   class="w-full border border-gray-300 rounded-md text-sm px-3 py-2">
                        </div>
                    </div>

                    <div class="flex items-center justify-end gap-3 pt-2">
                        <a href="{{ route('zonas.show', $zona->id) }}" class="text-sm text-gray-500 hover:text-gray-700">
                            Cancelar
                        </a>
                        <button type="submit"
